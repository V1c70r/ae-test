...
const returnIfIdEqual = id => node => {
  ...
  if (!idAttr || idAttr.value !== id) { return }
...
function getAttrsScore(originNode) {
  ...
  return node => {
    if (node.nodeName !== originNode.nodeName || !node.attrs) { return 0 }
 ...
 * @param {[string, number][]} path e.g. [['', 1], ['html', 1], ['body', 1], ['div', 2], ['div', 3]]
 * @returns {string} e.g. (/html/body/div[2]/div)[3]
 */
function pathToString(path) {
  ...
  return '(' +
    path
      .map(([name, num], i) => num > 1 && i < lastIndex ? `${name}[${num}]` : name)
      .join('/')
  + `)[${path[lastIndex][1]}]`
}
